		...
		for pts in collection_ord['points']:
			...
			if (collection_ord['color'][ii] == True):
				...
				if (ii == 0 or all(collection_ord['color'][j] == False for j in range(0,ii-1))):
					IC0 = np.array([-0.44+self.shiftX_Plink1_plate+self.shiftX_plate_R1link2, 0+self.shiftY_Plink1_plate+self.shiftY_plate_R1link2, 0+self.Zgripper]) # homing of arm
				else:
				...
				if (len_wpts3 != 0) and (self.Push_plane == True):		
				...
				if (len_wpts4 != 0 and self.Push_above == True):
				...
				if ((self.Push_plane == False) and (len(wpts3) != 0)):
					for wpt3 in wpts3:
						subset_Ncond.append(wpt3)
				if ((self.Push_above == False) and (len(wpts4) != 0)) :
					for wpt4 in wpts4:
				...
				return IC0, cluster_init, cond_pts, subset_Ncond, goal_update
