...
# This is advanced script to generate conditioned promp for franka, then the push_cost will create pushing actions (optimization with pushing cost, so NL constraints, is not working here)
...
class promp_pub(object):
	...
	def intprompV2_opt_sariah(self, condpromp, goal, cond_pts, tcond, inclination, Lstems, mean, cov):
		...
		LC, bounds = self.linear_constraints_sariah(trajectoryFlat, init_traj, mean, cov)
		...
		optimized_trajectory = opt.minimize(self.traj_opt.calculate_total_cost, trajectoryFlat, method='trust-constr', jac=self.traj_opt.cost_gradient_analytic, constraints = LC, options={'maxiter': 15, 'disp': True}, callback=self.optCallback, bounds = bounds)
		print('passed the optimization, opt traj=', optimized_trajectory)
		optimized_trajectory = np.transpose(optimized_trajectory.x.reshape((7, len(optimized_trajectory.x) / 7)))
		optimized_trajectory = np.insert(optimized_trajectory, 0, initial_joint_values, axis=0)
		optimized_trajectory = np.insert(optimized_trajectory, len(optimized_trajectory), desired_joint_values, axis=0)
