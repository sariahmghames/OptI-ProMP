...
import traj_SeqOpt as traj_opt  
...
class promp_pub(object):
	...
	def __init__(self):
		...
		self.qf = [0.8828870432781536, -0.396002139899204, -0.6750866635641583, -1.7443463146582736, 2.8925341042695, 1.7158042084282294, 0.8]
		...
		self.T0 = 0
		self.nDof = 7
		self.t1samples = 200 # was 100, the smallest the faster the optimization
		self.t2samples = 200 # was 100
		self.extra_cond = 2 # cam + goal
		self.nb_cond = 2 + self.extra_cond
		self.max_nb_cond = 10 
		self.time_normalised1 = np.linspace(self.T0, self.Tf, self.t1samples)
		self.t_cam2 = self.time_normalised1[-1]-0.1 # self.Tf - (self.nb_cond / self.max_nb_cond) 
		...
		self.constraints = {}
		self.goal = np.array([0.44999826901475964, 0.0, 0.89]) # init to straw81
		self.neighbours = []
		self.connections = []
	...
	def cluster_specifications(self,): 
		...
		straw81 = np.array([0.44999826901475964, 0.0 , 0.89, 4.244566712753927e-06]) # [0.44999826901475964, 0.10000205479846114 , 1.0599142533194694]
		straw82 = np.array([0.44999851284989234, 0.00385, 0.7541, 0.3133357533444485]) # y has changed wrt sim ; # [0.44999851284989234, 0.0961539977781866
		straw83 = np.array([0.4499984064530141, 0.00217, 0.8286, -0.32215970186415416]) # y has changed wrt sim; # 0.4499984064530141, 0.0978313002183852
		bunch_xyz = []
		bunch_col = []
		bunch_orient = []
		...
		g8_n1_so = (np.pi)/20 # was (np.pi)/20 for matching sim  
		g8_n2_so = -(np.pi)/20
	...
	def promp_generator(self, IC, goal):
		...
		# Later chosen combination was: num basis=5, width = 4, outside =1)  # if IC (q0) is straight up, and so choose Tf = 0.3 and t_cam = Tf -0.1
		# Later chosen combination was: num basis=5, width = 2, outside =5)
		basisGenerator1 = basis.NormalizedRBFBasisGenerator(phaseGenerator, numBasis=5, duration=self.Tf, basisBandWidthFactor=4, # check duration
		                                                   numBasisOutside=1) 
	...
	def optCallback(self, xk):   # xk is  joints traj, here we calculate total costs based on traj of joints; for BFGS remove state arg, for trust_region include state arg 
	...
	def nonlinear_constraints_sariah(self, theta):
		...
		init_traj = init_traj[self.tcam_samp:, :] # remove in case optimizing the 2nd half only of the traj
		r, c = init_traj.shape
		...
		for spls in range(len(init_traj)):
			...
			dist = pt1 - pt2
			d_proj_y = np.absolute(self.traj_opt.table_unit.dot(dist))
			d_proj_x = np.absolute(self.traj_opt.ptable_unit.dot(dist))
	...
	def intprompV2_opt_sariah(self, condpromp, goal, cond_pts, inclination, Lstems):
		self.constraints = {'goal': goal, 'condpts':cond_pts,'tcond':[], 'incl': inclination, 'lstems':Lstems }
		self.goal = goal
		...
		init_traj = condpromp # 200x7
		print('init traj=',init_traj[0,:])
		#init_traj = np.squeeze(init_traj) # 200x7
		bi, ri = init_traj.shape  # b: blocks = 100 (time), r = rows(7, dof) and c = columns (20, samples from the Gaussian distribution at each dof at an instant of time)
		print('traj dim is: {}, {}'.format(bi, ri))
		...
		if self.constraints:
			for spls in range(bi):
				...
				bounds_up.append([2.89725, 1.76278, 2.89725, -0.0698, 2.89725, 3.75246, 2.89725]) # joint4 : 0.0698132

		if self.constraints:
			if self.stage_opt == "push":
				print('Constraints are available')
				for dem in range(c): # consider ci and bi instead of c and b in case of optimizing only the 2nd half of the traj
					for spls in range(b):
						c1_lb.append(self.Rgrip)
						c2_lb.append(0)
						c3_lb.append(self.Rgrip)
						c1_ub.append(2.0*self.Rgrip)#np.inf
						c2_ub.append(0)
						c3_ub.append(2.0*self.Rgrip)

				lb = np.concatenate((c1_lb, c2_lb), axis=0)# c1_lb, c2_lb, c3_lb
				ub = np.concatenate((c1_ub, c2_ub), axis=0) #c1_ub, c2_ub, c3_ub
			...
			if self.stage_opt == "push":
				optimized_trajectory = opt.minimize(self.traj_opt.calculate_total_cost, trajectoryFlat, method='trust-constr', jac=self.traj_opt.cost_gradient_analytic, constraints = nLC, callback = self.optCallback, options={'maxiter': 10, 'disp': True})  # or try SLSQP, options={'xtol': 1, 'gtol': 1, 'maxiter': 10, 'disp': True}
			else:
				optimized_trajectory = opt.minimize(self.traj_opt.calculate_total_cost, trajectoryFlat, method='L-BFGS-B', jac=self.traj_opt.cost_gradient_analytic, bounds = bounds , callback=self.optCallback, options={'maxiter': 10, 'disp': True}) #'gtol': 2e-1,
...
if __name__ == "__main__":
	...
	with open('/home/sariah/intProMP_franka/src/TrajOpt_UoL/promp_trajopt/traj_opt/src/results/bounds/latest/push1_smooth0_max10-20-const2Rgrip_goalreachable/traj_init_opt.npz', 'r') as f:
		data = np.load(f, allow_pickle=True)
		traj_init = data[0]
		traj_init_opt = data[1]



	#intprompV2_generator.plot_traj(intprompV2_task, cond_pts, IC0, cluster_init, spherical, Lstems, cam =x_cam)
	intprompV2_Opt = intprompV2_generator.intprompV2_opt_sariah(traj_init_opt, cluster_init, min_wpts2, spherical, Lstems)
