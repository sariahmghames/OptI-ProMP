...
class promp_pub(object):
	...
	def __init__(self):
		...
		self.t_cam2 = self.time_normalised1[-1]-0.3
	...
	def promp_generator(self, IC, goal):
		...
		# Later chosen combination was: num basis=5, width = 2, outside =3)
	...
	def intprompV2_opt_sariah(self, condpromp, goal, cond_pts, tcond, inclination, Lstems, mean, cov):
		...
		if self.constraints:
			...
			optimized_trajectory = opt.minimize(self.traj_opt.calculate_total_cost, trajectoryFlat, method='trust-constr', jac=self.traj_opt.cost_gradient_analytic, constraints = nLC, callback = self.optCallback, options={'maxiter': 3, 'disp': True})  # or try SLSQP, options={'xtol': 1, 'gtol': 1, 'maxiter': 10, 'disp': True}
