...
class CostFunction(object):
    ...
    def get_arm_discretised_points(self, joint_values=None, step_size=0.2, with_joint_index=False):
    ...
    def compute_minimum_distance_to_objects(self, robot_body_points, object_list=None, ):
        ...
        for r in robot_body_points:  # expects robot_body_points as an array of dimension 1 x n
            ...
            for o in object_list:
                ...
                dist.append(norm_ro - o[3]-self.dsafe_agri) # S: 0.15m is a safety factor, - 0.15
            D.append(np.min(np.array(dist)))
        return D


    def compute_minimum_distance_to_objects_push(self, arm_body_points, object_list_push=None, ):
        if object_list_push is None:
            object_list_push = self.get_object_collision_spheres()
        D = list()
        if len(arm_body_points.shape) == 1:
            arm_body_points = arm_body_points.reshape(1, 3) # x, y , z vert stacked

        for r in arm_body_points:  # expects robot_body_points as an array of dimension 1 x n
            dist = []
