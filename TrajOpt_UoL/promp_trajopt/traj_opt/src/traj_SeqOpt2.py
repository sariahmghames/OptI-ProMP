...
from franka_Intpromp_noAPF_bounds_seqOpt2 import promp_pub 
import formulations as fm


class trajectory_optimization(promp_pub):

    def __init__(self, goal, const, tcam2, tsamples, tf, push_obs):
        ...
        #self.object_list_agri = np.array([[goal[0]-0.0, goal[1]-0.2, goal[2]-0.1, 0.1], [goal[0]+0.0, goal[1]+0.2, goal[2]-0.1, 0.1]]) 
        self.object_list_agri = np.array([[goal[0]-0.0, goal[1]-0.2, goal[2]-0.1, 0.1], [goal[0]+0.0, goal[1]+0.2, goal[2]-0.1, 0.1]])  # sphere
        self.object_list_bar = np.array([[goal[0], goal[1]+0.12, goal[2]-0.0, 0.0]])  
        self.object_push_list = push_obs
        self.initial_joint_values = np.zeros(7)  # + 0.01 * np.random.random(7)
        self.ang_deg = 60
        self.desired_joint_values = np.array([np.pi * self.ang_deg / 180, np.pi / 3, 0.0, np.pi / 6, np.pi / 6, np.pi / 6, np.pi / 6])
        self.optCurve = []
        self.step_size = 0.02
        self.push_elem = const 
        self.table_unit = np.array([0,1,0]) # was np.array([1,0,0]) when y_goal was 0.49 in robot base and x_goal was 0.1, so the 2 blue obs spheres where along x axis
        self.ptable_unit = np.array([1,0,0])
    ...
    def calculate_obstacle_cost(self, trajectory):  # traj is 7xn, eq (3)
        ...
        arm_vel_normalised, arm_vel_mag, arm_vel = self.calculate_normalised_armworkspace_velocity(
            trajectory) 
        for jvs in range(len(trajectory)):
            ...
            arm_discretised_points = np.array(
                self.cost_fun.get_arm_discretised_points(trajectory[jvs], self.step_size)) 
            dist = self.cost_fun.compute_minimum_distance_to_objects(robot_discretised_points, self.object_list_agri)
            distp= self.cost_fun.compute_minimum_distance_to_objects_push(arm_discretised_points, self.object_push_list)
            obsacle_cost_potential = np.array(self.cost_fun.cost_potential(dist)) # c(x,u) in CHOMP paper
            obsacle_cost_potential_push = np.array(self.cost_fun.cost_potential(distp))
            obstacle_cost += (np.sum(np.multiply(obsacle_cost_potential, vel_mag[jvs, :])) + np.sum(np.multiply(obsacle_cost_potential_push, arm_vel_mag[jvs, :])))#Eq (3) in CHOMP papern discritized formalization, += is over time and sum() is among body points (vel_mag and obstacle cost potential are vectors with nb of body pts dependency)
    ...
    def calculate_push_cost(self, trajectory):  # traj is (1400,)=(7*200), eq (3) , d along table top only (so dy)
        push_cost = 0
        ...
        #print('traj_sub shape =', traj_sub.shape)

        if self.constraints:
            #for jvs in range(len(trajectory)/2):
            #    push_cost += 0
            #for jvs in range(len(traj_sub)):
            for jvs in range(len(trajectory)): 
                T, Tj =  self.franka_kin.fwd_kin(trajectory[jvs, :])
                #T, Tj =  self.franka_kin.fwd_kin(traj_sub[jvs, :])
                dy = self.table_unit.dot(T[0:3,3] - self.push_elem['goal']) 
                #d_norm = np.linalg.norm(d)
                dy_norm = np.absolute(dy) 
                #push_cost += (dy_norm )
                push_cost += 1/dy_norm
        return push_cost
    ...
    def calculate_push_cost_dxy(self, trajectory):  # traj is (1400,)=(7*200), eq (3)
        push_cost = 0
        tcam_samp = str(np.round(self.t_cam2*(self.t1samples)/self.Tf,0))
        tcam_samp = tcam_samp.rstrip('0').rstrip('.') if '.' in tcam_samp else tcam_samp
        tcam_samp = int(tcam_samp)
        ...
        if self.constraints:
            ...
            for jvs in range(len(trajectory)): 
                ...
                #T, Tj =  self.franka_kin.fwd_kin(traj_sub[jvs, :])
                dy = self.table_unit.dot(T[0:3,3] - self.push_elem['goal']) 
                dx = self.ptable_unit.dot(T[0:3,3] - self.push_elem['goal']) 
                #d_norm = np.linalg.norm(d)
                dy_norm = np.absolute(dy) 
                dx_norm = np.absolute(dx)
                push_cost += (dy_norm + dx_norm)
                #push_cost += 1/d_norm
        return push_cost
    ...
    def calculate_total_cost(self, trajectory): # traj is flat
        ...
        #return 1.0 * push_cost + 1.0 * obstacle_cost 
        #return 1.0 * obstacle_cost # Eq (1) in CHOMP paper, 1 + 0.5 + 2 nop, 
        #return 1.0 * push_cost + 5.0 * smooth_cost # Eq (1) in CHOMP paper, 5 nop
        ...
        #return 1* push_cost # yes
        #return 1.0* push_cost + 1.0 * obstacle_cost # yes
        #return 1.0* obstacle_cost
        return 1* np.squeeze(smooth_cost)
        ##return 20* _cost
    ...
    def calculate_normalised_armworkspace_velocity(self, trajectory): # points on robot body during its movement to desired conf (dx/dt)
        # We have not divided by  time as this has been indexed and is thus not available
        trajectory = np.insert(trajectory, len(trajectory), self.desired_joint_values, axis=0)# insert at the end the desired values
        arm_body_points = np.array([self.cost_fun.get_arm_discretised_points(joint_values, self.step_size) for joint_values in trajectory])
        #print('size body pts=',robot_body_points.shape)
        velocity = np.diff(arm_body_points, axis=0)
        #print('workspace vel=', velocity.shape)
        vel_magnitude = np.linalg.norm(velocity, axis=2)
        vel_normalised = np.divide(velocity, vel_magnitude[:, :, None], out=np.zeros_like(velocity), where=vel_magnitude[:, :, None] != 0) # out zero normalized vel where magnitude is 0
        return vel_normalised, vel_magnitude, velocity
    ...
    def fun(self, points): # related to Eq (21)
        dist = self.cost_fun.compute_minimum_distance_to_objects(points, self.object_list_agri)
    ...
    def calculate_obstacle_cost_gradient(self, trajectory):
        ...
        for jvs in range(len(trajectory)): # len of time steps
            ...
            dist = self.cost_fun.compute_minimum_distance_to_objects(robot_discretised_points, self.object_list_agri,) # from each points on robot body in conf X to each object in scene (min dist arcross all obstacles)
    ...
    def calculate_push_cost_gradient(self, trajectory):
        ...
        #for spls in range((len(trajectory))/2):
        #    push_gradient.append(np.zeros(7))
        #for jvs in range(len(traj_sub)):
        for jvs in range(len(trajectory)): # len of time steps
            Tee, Tj = self.franka_kin.fwd_kin(trajectory[jvs,:]) 
            #Tee, Tj = self.franka_kin.fwd_kin(traj_sub[jvs,:]) 
            std_jacob = self.franka_kin.geometric_jacobian(Tj, Tee)
            Xee = Tee[0:3,3]
            Xg = self.goal
            #push_grad_false_y = (np.absolute(self.table_unit.T.dot(Xee - Xg))*((np.transpose(std_jacob[0:3,:])).dot(self.table_unit))) # if cost = d_absolte
            #push_grad_false_x = (np.absolute(self.ptable_unit.T.dot(Xee - Xg))*((np.transpose(std_jacob[0:3,:])).dot(self.ptable_unit))) # if cost = d_absolte, and cost_push = dx + dy
            push_grad_false_y = -(1*((np.transpose(std_jacob[0:3,:])).dot(self.table_unit)))/((np.absolute(self.table_unit.T.dot(Xee - Xg))**2))  # if cost = 1/d_absolte
            #push_gradient.append(push_grad_false_y+push_grad_false_x)
            push_gradient.append(push_grad_false_y)
    ...
    def cost_gradient_analytic(self, trajectory):  # calculate grad(cost) = grad(smoothness_cost) + grad(obstacle_cost)
        ...
        #cost_gradient = push_gradient.reshape((len(trajectory)/2, 1)) # for push cost under half traj consideration
        #cost_gradient = push_gradient.reshape((len(trajectory), 1)) + smoothness_gradient # for push cost under the whole traj consideration
        ...
        #cost_gradient = 1* obstacle_gradient.reshape((len(trajectory), 1)) + 1* push_gradient.reshape((len(trajectory), 1)) 
    ...
    def animation(self, optimized_trajectory, initial_trajectory, xf, cond_pts, sph, lstems, connections):  # initial_trajectory is from ProMP
        ...
        while True:
            ...
            for i in range(len(optimized_trajectory)):
                T_optim, T_joint_optim = self.franka_kin.fwd_kin(optimized_trajectory[i]) # T is traj ?
                Tee.append([T_optim[0,3], T_optim[1,3], T_optim[2,3]])
                

            for i in range(len(optimized_trajectory)):
                T_optim, T_joint_optim = self.franka_kin.fwd_kin(optimized_trajectory[i])  # T is traj ?
                T_init, T_joint_init = self.franka_kin.fwd_kin(initial_trajectory[i])
                ax.clear()
                ax.scatter(xf[0], xf[1], xf[2], s = 50, c='r', marker='o')
                for obs in self.object_list_agri:
                    self.sphere(ax, obs[3], [obs[0], obs[1], obs[2]])  # axis, radius , center, 0.15, 0.51, 0.2, 0.1
                for x in range(len(Tee)):
                    ax.scatter(Tee[x][0], Tee[x][1], Tee[x][2], s = 2, c='b', marker='o')
