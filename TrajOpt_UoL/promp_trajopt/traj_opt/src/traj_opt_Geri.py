...
from franka_Intpromp import promp_pub 


class trajectory_optimization(promp_pub):

    def __init__(self, const, tcam2, tsamples, tf):
        ...
        self.object_list = np.array([[0.15, 0.51, 0.2, 0.1]])  # for one sphere (x,y,z, radius)
    ...
    def calculate_push_cost(self, trajectory):  # traj is 1400=(7*200), eq (3)
        ...
        if self.constraints:
            ...
            for jvs in range(len(traj_sub)): 
                T, Tj =  franka_kin.fwd_kin(traj_sub[jvs, :])
                d = self.table_unit.dot(T[0:3,3] - self.push_elem['goal']) 
                d_norm = np.square(np.linalg.norm(d))
                push_cost += d_norm
        return push_cost
    ...
    def calculate_total_cost(self, trajectory):
        ...
        #manip_cost = self.calculate_manip_cost(trajectory)
        #return 5 * push_cost + 1.5 * obstacle_cost + manip_cost # Eq (1) in CHOMP paper
