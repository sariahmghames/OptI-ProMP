...
class promp_pub(object):
	...
	def __init__(self):
		...
		self.Tf = 0.8 # originally set to 0.3 --> chnage with task and with position of robot wrt cluster
		self.T0 = 0
		self.nDof = 7
		...
		self.t_cam2 = self.time_normalised1[-1]-0.4 # self.Tf - (self.nb_cond / self.max_nb_cond) 
	...
	def intprompV2_opt_sariah(self, condpromp, goal, cond_pts, inclination, Lstems):
		...
		if self.constraints:
			for spls in range(bi):
				...
				bounds_up.append([2.89725, 1.76278, 2.89725, -0.1, 2.89725, 3.75246, 2.89725]) # joint4 : 0.0698132
...
if __name__ == "__main__":
	...
	with open('/home/sariah/intProMP_franka/src/TrajOpt_UoL/promp_trajopt/traj_opt/src/results/bounds/latest/push1_smooth0_max10_const2Rgrip/traj_init_opt.npz', 'r') as f:
		data = np.load(f, allow_pickle=True)
		traj_init = data[0]
