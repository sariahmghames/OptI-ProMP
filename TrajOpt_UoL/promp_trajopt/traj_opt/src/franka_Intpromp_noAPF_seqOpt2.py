...
import traj_SeqOpt2 as traj_opt  
...
class promp_pub(object):
	...
	def intprompV2_opt_sariah(self, condpromp, goal, cond_pts, inclination, Lstems):
		...
		# 	print('lb shape=', np.asarray(lb).shape)
			...
			optimized_trajectory = opt.minimize(self.traj_opt.calculate_total_cost, trajectoryFlat, method='BFGS', jac=self.traj_opt.cost_gradient_analytic, callback=self.optCallback, options={'maxiter':10, 'disp': True}) #'gtol': 2e-1,
...
if __name__ == "__main__":
	...
	with open('/home/sariah/intProMP_franka/src/TrajOpt_UoL/promp_trajopt/traj_opt/src/results/bounds/latest/seqOpt_smooth0_obs1_max10_3cmsafety_20cmNeighdist/traj_init_opt.npz', 'r') as f:
		data = np.load(f, allow_pickle=True)
		traj_init = data[0]
