...
class promp_pub(object):
	...
	def intprompV2_opt_sariah(self, condpromp, goal, cond_pts, tcond, inclination, Lstems, mean, cov):
		...
		if self.constraints:
			...
			optimized_trajectory = opt.minimize(self.traj_opt.calculate_total_cost, trajectoryFlat, method='trust-constr', jac=self.traj_opt.cost_gradient_analytic, constraints = nLC, callback = self.optCallback, options={'xtol': 1, 'gtol': 1.5, 'maxiter': 15, 'disp': True})  # or try SLSQP
