...
class Hist1D:
    ...
    def _computeTargetPixels(self, image, color_space, channel):
        color_pixels = ColorPixels(image, num_pixels=1200)
...
class Hist1DPlot:
    ...
    def plot(self, ax):
        ...
        for s in range(0,len(color_samples)): # sariah modified
            ax.bar(color_samples[s], color_densities[s], width=width, color=colors[s]) # sariah modified
